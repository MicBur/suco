...
CoordinatorConfig CoordinatorConfig::load() {
    ...
    std::string history_db = cache_dir + "/history.db";
#ifdef _WIN32
    history_db = cache_dir + "\\history.db";
#endif
    if (const char* env_history_db = std::getenv("SUCO_HISTORY_DB")) {
        history_db = env_history_db;
    }
    config.set_history_db_path(resolve_path(history_db));

    if (const char* env_prefetch = std::getenv("SUCO_PREFETCH_JOBS")) {
        try {
            config.set_prefetch_jobs(static_cast<size_t>(std::stoul(env_prefetch)));
        } catch (...) {}
    }

    if (const char* env_threads = std::getenv("SUCO_COORDINATOR_BATCH_THREADS")) {
        try {
            config.set_coordinator_batch_threads(static_cast<size_t>(std::stoul(env_threads)));
        } catch (...) {}
    }
