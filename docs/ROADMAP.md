...
- [x] **Remote Preprocessing** (#42): Ship raw headers and source files to worker nodes for remote `-E` preprocessing, eliminating local client CPU overhead. ***DONE + grid-verified + CI-enforced** (opt-in `SUCO_REMOTE_PREPROCESS`): eligible TUs skip the local `-E` entirely — the client builds a project-header bundle (cheap `-MM`), the worker preprocesses RAW source + bundle, objects are byte-identical/deterministic. Time-macro/module TUs fall back. Remaining before default-on: bundle dedup + a real-project byte-identity sweep.*
