...
- [x] **Remote Preprocessing** (#42): Ship raw headers and source files to worker nodes for remote `-E` preprocessing, eliminating local client CPU overhead. ***DONE — now ON by default**, verified on both Linux (240/240 objects byte-identical to native) and a real Windows→Linux PE/COFF cross-compile, plus a 101-TU Windows project that builds, links and runs. Eligible TUs skip the local `-E` entirely (**~42 % less client CPU**); time-macro/C++20-module TUs fall back automatically. Both paths are CI-pinned; `SUCO_REMOTE_PREPROCESS=0` restores classic preprocessing. Details: [remote_preprocessing_verification.md](remote_preprocessing_verification.md).*
